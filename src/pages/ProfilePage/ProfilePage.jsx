import { Box, Card, Typography, FormControl, InputLabel, OutlinedInput, Avatar, TextField, Button } from "@mui/material";
...
export default function ProfilePage({ data }) {
    ...
    const handleSubmitForm = () => {
        ...
        console.log(values)
        axios.post(`${BASE_URL}/api/user/v1/update`, {...values}, {headers : authHeader() }).then((res) => console.log(res))
    }
    ...
    return (
        <div className="grid place-content-center h-screen w-full flex-1 pb-80">
            <Card
                sx={{
                    width: { sm: "400px", lg: "650px", xs: "400px", borderRadius: "20px" },
                }}
                className="w-screen"
            >
                    ...
                    <Box className={`items-center min-h-80 pb-12 ${edit ? 'hidden' : 'flex flex-col'}`}>
                        <Typography fontSize={24} fontWeight='bold'>{data?.firstName} {data?.lastName}</Typography>
                ...
                <Box className={`${edit ? 'flex flex-col' : 'hidden'} items-center min-h-80 pb-12`}>
                    ...
                    <Button
                        className="!mt-20"
                        type="submit"
                        onClick={() => {handleChangeEdit(false); handleSubmitForm()}}
                        variant="contained"
                        sx={{
                            borderRadius: "8px",
                            padding: "5px 60px",
